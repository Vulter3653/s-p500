#!/usr/bin/env python3
"""Build the ordered 2023 S&P 500 manifest using exact reportDate 10-Ks."""
...
def eligible_filing(client: SecClient, cik: str) -> tuple[dict | None, str]:
    ...
    for fragment in response.get("filings", {}).get("files", []):
        ...
        if start and end and (end < "2023-01-01" or start > "2026-07-30"):
            continue
    ...
    if len(candidates) != 1:
        reason = (
            "no_unique_eligible_2023_report_date_10k"
            if not candidates
            else "ambiguous_multiple_eligible_2023_report_date_10k"
        )
        return None, reason
    row = candidates[0]
    return {
...
def build(root: Path, output: Path) -> None:
    universe_path = root / "2023/sp500_companies.csv"
    ...
    client = SecClient(
        root / "2023/sample_500/cache/sec_submissions",
        root / "2023/sample_500/metadata/sec_requests.jsonl",
    )
    accepted, excluded = [], []
    for universe_order, company in enumerate(universe, 1):
        ...
        if not raw_cik:
            excluded.append({
                ...
                "cik": "",
                "reason": "cik_missing_in_2023_universe",
            })
            continue
    ...
    for sample_order, (universe_order, company, filing) in enumerate(accepted, 1):
        ...
        company_id = f"S2023-{sample_order:03d}"
        rows.append({
            ...
            "source_manifest": "2023/sp500_companies.csv",
            "sample_group": "sample_2023",
            "batch_id": ((sample_order - 1) // 100) + 1,
            "r2_object_key": (
                f"2023/sample_500/html/raw/{cik}/{accession}.html"
            ),
            "universe_order": universe_order,
        })
    ...
    if any(
        row["form"] != "10-K"
        or not row["report_date"].startswith("2023-")
        or not row["filing_url"]
        for row in rows
    ):
        raise ValueError("non-eligible filing in 2023 manifest")

    write_csv(output, rows, fields)
    write_csv(
        output.parent / "quality_check/excluded_companies.csv",
        excluded,
        ["universe_order", "_company_key", "ticker", "company_name", "cik", "reason"],
    )
    print(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("2023/sample_500/sample_manifest_2023_500.csv"),
    )
