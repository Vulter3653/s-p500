#!/usr/bin/env python3
"""Create repository 2022 results from the five completed batch artifacts."""
...
def merge(root: Path, artifacts: Path) -> dict:
    sample = root / "2022/sample_500"
    _, manifest = read_csv(sample / "sample_manifest_2022_500.csv")
    order = {row["company_id"]: int(row["sample_order"]) for row in manifest}
    batch_roots = [
        artifacts / f"10k-2022-batch-{batch}" for batch in range(1, 6)
    ]
    ...
    merged_root = artifacts / "10k-2022-sample_500-merged-results"
    ...
    summary = {
        ...
        "workflow_url": (
            "https://github.com/Vulter3653/s-p500/actions/runs/30533585643"
        ),
        **batch_rows,
