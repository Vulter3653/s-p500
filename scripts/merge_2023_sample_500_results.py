#!/usr/bin/env python3
"""Create repository 2023 results from the five completed batch artifacts."""
...
def merge(root: Path, artifacts: Path) -> dict:
    sample = root / "2023/sample_500"
    _, manifest = read_csv(sample / "sample_manifest_2023_500.csv")
    order = {row["company_id"]: int(row["sample_order"]) for row in manifest}
    batch_roots = [
        artifacts / f"10k-2023-batch-{batch}" for batch in range(1, 6)
    ]
    ...
    merged_root = artifacts / "10k-2023-sample_500-merged-results"
    ...
    summary = {
        ...
        "workflow_url": (
            "https://github.com/Vulter3653/s-p500/actions/runs/30532069306"
        ),
        **batch_rows,
