#!/usr/bin/env python3
"""Build the ordered 2022 S&P 500 manifest using exact reportDate 10-Ks."""
...
def eligible_filing(
    ...
    for fragment in response.get("filings", {}).get("files", []):
        ...
        if start and end and (end < "2022-01-01" or start > "2026-07-30"):
            continue
    ...
    if len(candidates) != 1:
        reason = (
            "no_unique_eligible_2022_report_date_10k"
            if not candidates
            else "ambiguous_multiple_eligible_2022_report_date_10k"
        )
...
def build(root: Path, output: Path) -> None:
    universe_path = root / "2022/sp500_companies.csv"
    ...
    client = SecClient(
        root / "2022/sample_500/cache/sec_submissions",
        root / "2022/sample_500/metadata/sec_requests.jsonl",
    )
    accepted, excluded = [], []
    for universe_order, company in enumerate(universe, 1):
        ...
        if not raw_cik:
            excluded.append({
                ...
                "cik": "",
                "reason": "cik_missing_in_2022_universe",
    ...
    for sample_order, (universe_order, company, filing) in enumerate(accepted, 1):
        ...
        company_id = f"S2022-{sample_order:03d}"
        rows.append({
            ...
            "source_manifest": "2022/sp500_companies.csv",
            "sample_group": "sample_2022",
            "batch_id": ((sample_order - 1) // 100) + 1,
            "r2_object_key": (
                f"2022/sample_500/html/raw/{cik}/{accession}.html"
            ),
            "universe_order": universe_order,
        })
    ...
    if any(
        row["form"] != "10-K"
        or not row["report_date"].startswith("2022-")
        or not row["filing_url"]
        for row in rows
    ):
        raise ValueError("non-eligible filing in 2022 manifest")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("2022/sample_500/sample_manifest_2022_500.csv"),
    )
