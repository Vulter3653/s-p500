#!/usr/bin/env python3
"""Create repository 2024 results from the five completed batch artifacts."""
...
def merge(root: Path, artifacts: Path) -> dict:
    sample = root / "2024/sample_500"
    _, manifest = read_csv(sample / "sample_manifest_2024_500.csv")
    order = {row["company_id"]: int(row["sample_order"]) for row in manifest}
    batch_roots = [
        artifacts / f"10k-2024-batch-{batch}" for batch in range(1, 6)
    ]
    ...
    merged_root = artifacts / "10k-2024-sample_500-merged-results"
    ...
    summary = {
        ...
        "workflow_url": (
            "https://github.com/Vulter3653/s-p500/actions/runs/30530297712"
        ),
        **batch_rows,
