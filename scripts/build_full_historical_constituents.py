...
The historical-components CSV is the membership source of truth. SEC ticker
metadata is only an enrichment source and is resolved cache-first.
"""
...
def load_history(content: bytes) -> pd.DataFrame:
    ...
    return history.sort_values("date", kind="stable").drop_duplicates("date", keep="last").reset_index(drop=True)
...
def supported_years(history: pd.DataFrame, start_year: int | None, end_year: int | None) -> list[int]:
    ...
    earliest_supported = earliest.year - 1 if (earliest.month, earliest.day) == (1, 1) else earliest.year
    latest_supported = latest.year - 1 if (latest.month, latest.day) == (1, 1) else latest.year
    ...
    if first > last:
        raise ValueError(f"requested range has no source-supported years: {start_year}-{end_year}")
...
def metadata_lookup(current: pd.DataFrame, sec_map: dict[str, dict[str, str]]) -> dict[str, dict[str, str]]:
    ...
    for symbol, values in sec_map.items():
        lookup.setdefault(symbol, {
            "symbol": symbol, "security": values.get("security", ""),
            "gics_sector": "", "gics_sub_industry": "", "headquarters": "",
            "date_added": "", "cik": values.get("cik", ""), "founded": "",
            "metadata_status": "sec_current_ticker_snapshot",
        })
    for symbol, (security, cik) in HISTORICAL_TICKER_METADATA.items():
        lookup.setdefault(symbol, {
            "symbol": symbol, "security": security, "gics_sector": "",
            "gics_sub_industry": "", "headquarters": "", "date_added": "",
            "cik": cik, "founded": "", "metadata_status": "curated_legacy_ticker_map",
        })
    return lookup


def build_rows(year: int, snapshot: pd.Timestamp, source_row_date: pd.Timestamp,
               tickers: list[str], lookup: dict[str, dict[str, str]]) -> pd.DataFrame:
    return pd.DataFrame([
        {
            "sample_year": year,
            ...
            "symbol": symbol,
            "security": lookup.get(symbol, {}).get("security", ""),
            "gics_sector": lookup.get(symbol, {}).get("gics_sector", ""),
            "gics_sub_industry": lookup.get(symbol, {}).get("gics_sub_industry", ""),
            "headquarters": lookup.get(symbol, {}).get("headquarters", ""),
            "date_added": lookup.get(symbol, {}).get("date_added", ""),
            "cik": lookup.get(symbol, {}).get("cik", ""),
            "founded": lookup.get(symbol, {}).get("founded", ""),
            "metadata_status": lookup.get(symbol, {}).get("metadata_status", "historical_membership_only"),
...
def company_frame(securities: pd.DataFrame) -> pd.DataFrame:
    ...
    columns = [column for column in securities.columns]
    aggregations = {
        column: ("first" if column in {"sample_year", "snapshot_date", "historical_source_date", "cik", "membership_source"} else join_unique)
...
def main() -> None:
    ...
    lookup = metadata_lookup(normalize_components(load_components(wiki_content)), sec_ticker_map(sec_content))
    ...
    for year in years:
        ...
        company_frame(securities).to_csv(companies_path, index=False, quoting=csv.QUOTE_MINIMAL)
        audit_rows.append({
            "sample_year": year, "snapshot_date": snapshot.date().isoformat(),
            "historical_source_date": source_row_date.date().isoformat(),
            "security_rows": len(securities), "company_rows": len(company_frame(securities)),
    ...
    manifest = {
        "generated_on": date.today().isoformat(), "source_retrieved_on": args.source_date,
        "supported_first_year": years[0], "supported_last_year": years[-1],
        "requested_start_year": args.start_year, "requested_end_year": args.end_year,
        "overwrite_existing": args.overwrite_existing, "skipped_existing_years": skipped_existing,
        ...
        "wikipedia_url": SOURCE_URL, "wikipedia_sha256": hashlib.sha256(wiki_content).hexdigest(),
        "sec_tickers_url": SEC_TICKERS_URL, "sec_tickers_sha256": sec_metadata["source_sha256"],
