#!/usr/bin/env python3
"""Build the ordered 2020 S&P 500 manifest using exact reportDate 10-Ks."""
...
def eligible_filing(
    ...
    for fragment in response.get("filings", {}).get("files", []):
        ...
        if start and end and (end < "2020-01-01" or start > "2026-07-30"):
            continue
    ...
    if len(candidates) != 1:
        reason = (
            "no_unique_eligible_2020_report_date_10k"
            if not candidates
            else "ambiguous_multiple_eligible_2020_report_date_10k"
        )
...
def build(root: Path, output: Path) -> None:
    universe_path = root / "2020/sp500_companies.csv"
    ...
    client = SecClient(
        root / "2020/sample_500/cache/sec_submissions",
        root / "2020/sample_500/metadata/sec_requests.jsonl",
    )
    accepted, excluded = [], []
    for universe_order, company in enumerate(universe, 1):
        ...
        if not raw_cik:
            excluded.append({
                ...
                "cik": "",
                "reason": "cik_missing_in_2020_universe",
                "candidate_accessions": "",
                "candidate_filing_dates": "",
                "candidate_primary_documents": "",
            })
            continue
        cik = normalize_cik(raw_cik)
        filing, reason, candidates = eligible_filing(client, cik)
        if filing is None:
            excluded.append({
                "universe_order": universe_order,
                "_company_key": company["_company_key"],
                ...
                "candidate_accessions": "|".join(
                    str(row.get("accessionNumber", "")) for row in candidates
                ),
                "candidate_filing_dates": "|".join(
                    str(row.get("filingDate", "")) for row in candidates
                ),
                "candidate_primary_documents": "|".join(
                    str(row.get("primaryDocument", "")) for row in candidates
                ),
            })
            continue
    ...
    for sample_order, (universe_order, company, filing) in enumerate(accepted, 1):
        ...
        company_id = f"S2020-{sample_order:03d}"
        rows.append({
            ...
            "source_manifest": "2020/sp500_companies.csv",
            "sample_group": "sample_2020",
            "batch_id": ((sample_order - 1) // 100) + 1,
            "r2_object_key": (
                f"2020/sample_500/html/raw/{cik}/{accession}.html"
            ),
            "universe_order": universe_order,
        })
    ...
    if [int(row["sample_order"]) for row in rows] != list(range(1, len(rows) + 1)):
        raise ValueError("sample order is not contiguous")
    if any(
        row["form"] != "10-K"
        or not row["report_date"].startswith("2020-")
        or not row["filing_url"]
        for row in rows
    ):
        raise ValueError("non-eligible filing in 2020 manifest")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("2020/sample_500/sample_manifest_2020_500.csv"),
    )
