...
- Docs: README·인수인계·historical run summary를 VERSION 0.14.0, 핵심 패널 2,829행, historical candidate 4,897행 및 backfill completed/PASS 상태와 일치시켰다. (codex)
- 보호: 기존 CSV·JSON·HTML·분석값·웹 소스는 변경하거나 재계산하지 않았고 SEC·WRDS·R2·Google Drive 요청도 수행하지 않았다. (codex)

## 2026-08-06 - 2006–2019 R2→Google Drive 이전 완료\n\n- 실행: workflow run `31098818002`가 성공했다. live R2 inventory 2,068개를 기존 `legacy_nested` 구조로 이전했다. (codex)\n- 결과: uploaded 2,068, skipped 0, conflict/failed 0, Drive file IDs 2,068, size·SHA verified 2,068, 총 10,042,667,905 bytes. (codex)\n- 경로: `<root>/<year>/sample_500/html/raw/<CIK>/<accession>.html`; 2020–2025 기존 파일 형식과 동일하다. (codex)\n- 보호: R2 objects deleted 0, overwritten 0; 기존 패널·분석·Figure·dashboard는 변경하지 않았다. (codex)\n- artifact: `historical-r2-google-drive-migration`, artifact ID `8967660533`. (codex)\n\n## 2026-08-06 - 2020–2025 Google Drive 기존 이전 확인\n\n- 확인: 성공 run `30544560261` artifact `r2-google-drive-migration-full`에서 2020–2025 2,829개가 업로드 2,828 + 기존 일치 1로 처리되었고, 2,829개 모두 size·SHA 검증 PASS였다. (codex)\n- 실제 기존 Drive 경로: `<root>/<year>/sample_500/html/raw/<CIK>/<accession>.html`; 파일명은 accession `.html`이다. 이는 legacy_nested 형식이며, 최근 실패 run의 `year_flat` 시도와 다르다. (codex)\n- 정정: full run `31096775253`은 기존 Drive를 확인하지 못한 것이 아니라, R2에서 2020–2025 source object를 다시 읽으려다 2,829개 404로 실패한 것이다. (codex)\n- 보호: R2 delete/overwrite 및 Drive 재업로드는 수행하지 않았다. (codex)\n\n## 2026-08-06 - R2→Google Drive 이전 대상 불일치\n\n- 실행: migration test run `31096629864`는 재실행 skip 검증에서 실패했다. full run `31096775253`은 2,829개 대상 전체가 처리되었으나 업로드 0, 검증 0, `missing_r2_object:404` 2,829개로 실패했다. (codex)\n- R2 read-only inventory run `31094282311` 기준 실제 객체는 2006–2019, 총 2,068개·10,042,667,905 bytes이며 2020–2025 prefix는 확인되지 않았다. (codex)\n- 보호: R2 삭제·overwrite, Google Drive 파일 생성 완료 판정, 기존 패널·분석·dashboard 변경은 수행하지 않았다. (codex)\n- 다음: 실제 R2 inventory에서 전체 object key·size·SHA를 생성한 manifest를 기준으로 기존 migration script를 다시 호출해야 한다. (codex)\n\n## 2026-08-06 - README 감사 결과 반영\n\n- 변경: README에 구성종목 외부 원천 감사 문서 링크, 2025 CIK 비교 수치, datasets 현재 snapshot 및 hanshof historical 비교 한계를 추가했다. historical 순서를 2017에서 임의 중단하지 않는다고 명시했다. (codex)\n- 보호: 기존 패널·분석·Figure·원본 HTML·R2·Google Drive는 변경하지 않았다. (codex)\n\n## 2026-08-06 - 구성종목 비교 문서 누락 점검\n\n- 확인: 기존 감사 문서에 외부 원천 revision 고정, raw header/SHA, hanshof historical 전체 비교 여부가 명시되지 않은 부분이 있어 누락·미확인 표를 추가했다. (codex)\n- 결론: 현재 정량 비교 범위는 내부 2020–2025 구조와 2025 대 datasets current snapshot이며, hanshof historical 전체 비교는 미실시 상태로 유지한다. (codex)\n\n## 2026-08-06 - 2020–2025 구성종목 외부 원천 비교 감사\n\n- 추가: `docs/constituent-source-comparison-2020-2025.md`에 프로젝트 연도별 CSV 행 수·snapshot date·CIK 중복, collection-ready manifest 유효 행 수, datasets 현재 파일의 2025 CIK 비교(488 교집합/9 local-only/11 external-only), hanshof historical raw 접근 한계를 기록했다. (codex)\n- 보호: `panel_2020_2025/`, `analysis/descriptive_2020_2025/`, 기존 Figure·원본 HTML·R2·Google Drive는 수정하지 않았다. (codex)\n- 검증: 로컬 Python 재현 명령으로 2020–2025 CSV 각 500행, snapshot date 및 historical reference 2,718행을 확인했다. 문서 전용 변경이며 분석·수집 재실행은 하지 않았다. (codex)\n\n## 2026-08-06 - historical published-file recovery\n\n- Confirmed the backfill chain completed through 2006 (`zero_streak=3`, 4,897 candidate rows). (codex)\n- Recovery run `31093446280` regenerated 78 missing analysis/source files and dashboard metadata; PR #39 was merged to main. (codex)\n- R2/Drive migration remains separately guarded; no external storage writes were performed. (codex)\n\n## 2026-08-06 - historical publication and R2 migration safeguards\n\n- Changed: historical annual publication now persists `analysis/historical_candidate` source tables and a verified per-year R2 migration manifest. (codex)\n- Changed: added a guarded manual workflow for up to three completed years; dry-run performs no external writes, and R2 deletion requires Drive verification plus explicit confirmation. (codex)\n- Validation: local YAML/Python checks are pending in Actions; existing 2020–2025 artifacts remain protected. (codex)\n\n## 2026-08-03 — Codespace handoff for active run (codex)
...
- 정적 문구·데이터·문서·산출물 일관성 확인과 빌드 및 실행 가능한 로컬 검증을 완료했으며 로컬 커밋 `a3fe2b0`, 생성 산출물 갱신 커밋 `420a4ff`로 기록함.
- 당시 기록 시점에는 push·PR·배포를 수행하지 않았으며, 이후 `origin/codex/web-research-report`에 push함. PR·배포는 수행하지 않음.
## 2026-08-06 — 원격 브랜치 push 완료 (codex)

- 로컬 변경사항을 `origin/codex/web-research-report`에 push함.
- 원격 기준 commit: `542b437`.
- PR 생성 및 배포는 수행하지 않음.
- 배포 후 브라우저 검증은 사용자가 직접 수행 예정.
## 2026-08-06 — 1차 사용자 피드백 공개 화면 단순화 (codex)

- 버전: 공개 보고서 기능 변경으로 프로젝트 버전을 `0.14.0`으로 올리고 `VERSION`·루트 package metadata·CHANGELOG를 동기화했다. 비공개 `web` 패키지 버전은 배포 버전이 아니므로 유지했다.
