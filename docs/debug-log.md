## 2026-08-06 - historical R2 inventory Google Drive 이전 성공\n\n- workflow: `migrate-historical-r2-inventory-to-google-drive.yml`, run `31098818002`. 첫 시도는 직렬 R2 HEAD가 오래 걸려 취소했고, bounded parallel HEAD 검증으로 재실행했다. (codex)\n- inventory: 2006–2019 총 2,068 objects, 10,042,667,905 bytes. (codex)\n- migration: `legacy_nested`, uploaded=2,068, skipped=0, conflicts=0, failed=0, invalid=0, verified_size_and_sha=2,068. (codex)\n- 안전성: R2 delete=0, overwrite=0, SEC recollection=0, language rerun=0. (codex)\n- artifact: ID `8967660533`; 연도별 summary와 manifest를 보존했다. (codex)\n- 해결: 기존 2020–2025 Drive 파일과 동일한 경로·파일명 규칙으로 나머지 historical raw HTML 이전을 완료했다. (codex)\n\n## 2026-08-06 - 2020–2025 Google Drive 이전 결과 우선 확인\n\n- 근거: migration run `30544560261` artifact `8761746401`의 `run_summary.md`, `migration_quality_summary.csv`, `raw_html_google_drive_manifest.csv`를 읽었다. (codex)\n- 결과: source 2,829행, uploaded 2,828, skipped_existing_match 1, failed 0, verified_size_and_sha 2,829. 연도별 valid rows는 446/462/471/479/487/484였다. (codex)\n- 위치·명칭: manifest의 Drive folder ID 필드가 `year/sample_500/html/raw/CIK` 체인을 가리키며 `drive_file_name`은 accession `.html`이다. 따라서 이미 이전된 자료는 `legacy_nested`다. (codex)\n- 최근 실패와의 관계: run `31096775253`은 2020–2025 R2 manifest를 재사용해 2,829개를 R2 HEAD했으나 모두 `missing_r2_object:404`였다. Google Drive 기존 파일 검증 단계까지 도달하지 못했다. (codex)\n- 안전 결과: 기존 Drive 파일은 수정·삭제하지 않았고 R2 삭제도 0이다. (codex)\n\n## 2026-08-06 - R2→Google Drive full migration 대상 목록 불일치\n\n- 재현: `gh workflow run migrate-r2-html-to-google-drive.yml --ref main -f mode=full -f drive_layout=year_flat`; run `31096775253`. (codex)\n- 관찰: source manifest rows 2,829; 2020–2025별 446/462/471/479/487/484행. 결과는 uploaded=0, skipped=0, verified=0, failed=2,829였다. 실패 파일은 모두 `missing_r2_object:404`였다. (codex)\n- 원인: workflow가 저장소의 2020–2025 `sample_500/r2_storage/html_r2_manifest.csv`를 읽었지만, read-only R2 inventory run `31094282311`의 실제 prefix는 `2006/sample_503/...`부터 `2019/sample_503/...`까지였다. 대상 목록과 R2 실제 보관 범위가 불일치했다. (codex)\n- 실제 inventory: 2006–2019 총 2,068 objects, 10,042,667,905 bytes. audit artifact `8964788905`에 연도별 object_count·total_bytes·head sample과 SHA metadata가 있다. (codex)\n- 안전 결과: R2 delete=0, R2 overwrite=0, Drive 이전 성공 판정=0. (codex)\n- 후속: 실제 R2 전체 목록을 object key·size·SHA·CIK/accession 기준으로 manifest화한 후 existing migration script의 `--manifest` 경로를 사용한다. `year_flat` 명칭을 유지하려면 company/sample identity 매핑을 먼저 검증해야 한다. (codex)\n\n## 2026-08-06 - README 감사 결과 누락\n\n- 문제: 구성종목 비교 감사 문서는 main에 있었지만 README가 이를 링크하지 않았고, README의 historical 처리 문구가 과거 상태를 유지하고 있었다. (codex)\n- 수정: README에 감사 링크·488/9/11 CIK 비교·외부 snapshot 한계·역순 historical 처리 원칙을 반영했다. (codex)\n\n## 2026-08-06 - 구성종목 감사 문서 누락 점검\n\n- 재점검: `docs/constituent-source-comparison-2020-2025.md`의 외부 revision SHA, raw header/SHA, 연도별 외부 비교, hanshof historical 전체 대조 기록을 항목별로 확인했다. (codex)\n- 수정: 문서에 `누락·미확인 항목 점검 결과`와 후속 재현성 요구를 추가했다. 미실시 비교를 일치 또는 0으로 대체하지 않았다. (codex)\n- 보호: 패널·분석·Figure·원본 HTML·R2·Drive·dashboard data는 변경하지 않았다. (codex)\n\n## 2026-08-06 - 2020–2025 구성종목 외부 원천 비교\n\n- 범위: 프로젝트 `2020–2025/sp500_companies.csv`와 collection-ready manifest를 두 외부 GitHub 원천과 비교했다. (codex)\n- 확인: 각 연도 CSV 500행; snapshot date는 2021-01-01부터 2026-01-01; CIK 중복 초과 행은 40, 34, 21, 14, 9, 2였다. manifest 유효 행은 446, 462, 471, 479, 487, 484이다. (codex)\n- 정량 비교: 2025 CSV와 datasets current `constituents.csv`는 500 대 502행, CIK 교집합 488, local-only 9, external-only 11이다. 외부 파일에 2026 유효일이 있어 기준일 불일치로 판정했다. (codex)\n- 제한: hanshof historical raw CSV는 이 환경의 4MB 응답 제한을 초과해 전체 행 대조하지 못했다. 이를 미확인 상태로 기록하고 일치한다고 주장하지 않았다. (codex)\n- 결과: `docs/constituent-source-comparison-2020-2025.md`에 범위·식별자 규칙·수치·재현 명령·추가 확인 항목을 기록했다. (codex)\n\n## 2026-08-06 - missing historical analysis files\n\n- Cause: annual publication commits before the source-persistence fix contained dashboard JSON references to `analysis/historical_candidate` files that were absent from main. (codex)\n- Recovery: regenerated tables and figures from `panel_historical_candidate/firm_year_language_panel_full_candidate.parquet`, validated all source-manifest paths, and merged the restoration branch. (codex)\n- Separate migration dry-run was safely blocked because historical R2 migration manifests were not yet persisted; no R2/Drive mutation occurred. (codex)\n\n## 2026-08-06 - historical source manifest and migration gap\n\n- Problem: published `web/public/data/source-manifest.json` referenced `analysis/historical_candidate` files that were not committed, and no historical R2-to-Drive workflow consumed the yearly collection manifests. (codex)\n- Fix: annual workflow now commits and validates analysis source tables, builds a persistent SHA-pinned migration manifest, and exposes a confirmation-gated migration workflow that reuses the existing verifier and deletion script. (codex)\n- Safety: dry-run performs no SEC, R2, Drive, panel, or dashboard writes; deletion is only available after verified Drive migration. (codex)\n\n## 2026-08-03 — Codespace termination handoff (codex)
