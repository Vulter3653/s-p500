...
function Report({ data }) {
  const { summary, definitions, yearly, descriptive, comparison, within, changes, pearson, spearman, vif, sampleAudit, quality, sourceManifest, buildMetadata, figureData, figureManifest, docs = {}, supplementalErrors = {} } = data;
  ...
  return <main className="report-app">
    ...
    <div className="report-main">
      ...
      <Section id="results" number="7" title="분석 결과" kicker="RESULTS"><h3>7.1 표본 특성 및 AI 공시 확산</h3><p>{first.report_year || "-"}년 AI 공시 비율은 {formatPercent(first.ai_disclosure_rate)}이고 {last.report_year || "-"}년에는 {formatPercent(last.ai_disclosure_rate)}이다. 연도별 관측치 수가 다르므로 전체 평균은 연도 평균의 단순평균이 아니라 원자료 전체 N을 기준으로 한다.</p>{supplementalErrors.yearly ? <SupplementalError source="/data/yearly-statistics.json" /> : <MetricTable caption="표 2. 연도별 핵심 기술통계" rows={resultRows} columns={[["report_year", "보고연도"], ["ai_disclosure_rate", "AI 공시 비율"], ["mean_ai_sentence_count_all", "AI 문장 평균(전체)"], ["mean_ai_sentence_count_disclosers", "AI 문장 평균(공시)"], ["mean_whole_report_concreteness_all", "전체 보고서 구체성"], ["mean_ai_concreteness_disclosers", "AI 구체성(공시)"], ["mean_present_tense_share_all", "현재 시제"], ["mean_future_tense_share_all", "미래 표지"], ["mean_lm_uncertainty_share_all", "Loughran–McDonald uncertainty"], ["mean_passive_voice_sentence_share_all", "수동태"], ["mean_fog_index_all", "Fog Index"], ["mean_report_word_count_all", "보고서 단어 수"]]} source="analysis/descriptive_2020_2025/tables/table_04_descriptive_statistics_by_year.csv" condition="AI 수준 값은 공시 firm-year 조건부" />}<h3>7.2 전체 기술통계</h3>{supplementalErrors.descriptive ? <SupplementalError source="/data/descriptive-statistics.json" /> : <MetricTable caption="표 3. 전체 표본 기술통계" rows={descriptiveRows} columns={[["variable", "변수"], ["N", "N"], ["mean", "평균"], ["standard_deviation", "표준편차"], ["p25", "제1사분위수"], ["median", "중앙값"], ["p75", "제3사분위수"]]} source="analysis/descriptive_2020_2025/tables/table_02_overall_descriptive_statistics.csv" condition="전체 firm-year; AI 수준 변수는 유효 분모 조건부" />}<h3>7.3 공시·미공시 비교</h3>{supplementalErrors.comparison ? <SupplementalError source="/data/disclosure-comparison.json" /> : <MetricTable caption="표 4. AI 공시 여부별 단순 비교" rows={comparisonRows} columns={[["variable", "변수"], ["disclosure_N", "공시 N"], ["disclosure_mean", "공시 평균"], ["non_disclosure_N", "미공시 N"], ["non_disclosure_mean", "미공시 평균"], ["mean_difference", "평균 차이"], ["standardized_mean_difference", "표준화 차이"], ["welch_t", "Welch t"], ["welch_pvalue", "p-value"]]} source="analysis/descriptive_2020_2025/tables/table_05_ai_disclosure_group_comparison.csv" condition="연도·산업·규모 통제 없는 단순 집단 비교" />}<h3>7.4 상관관계</h3>{supplementalErrors.pearson ? <SupplementalError source="/data/pearson-correlations.json" /> : <MetricTable caption="표 5. Pearson 상관관계 일부" rows={corrRows} columns={[["variable", "행 변수"], ["ai_disclosure", "AI 공시"], ["ai_sentence_count", "AI 문장 수"], ["whole_report_concreteness", "전체 구체성"], ["lm_uncertainty_share", "Loughran–McDonald uncertainty"], ["fog_index", "Fog Index"], ["report_word_count", "보고서 단어 수"]]} source="analysis/descriptive_2020_2025/tables/table_08_pearson_correlation_full_sample.csv" condition="pairwise complete observations" />}<h3>7.5 동일 기업 변화와 VIF</h3><div className="two-column"><div>{supplementalErrors.within ? <SupplementalError source="/data/within-firm-changes.json" /> : <p>동일 기업 내 변화 산출물은 {formatInteger(within?.length)}개 변수·연도 조합이며, 실제 전년도 관측치가 있는 pair만 포함한다. 전체 변화표는 <code>year-over-year-changes.json</code>에서 연결한다.</p>}</div>{supplementalErrors.vif ? <SupplementalError source="/data/vif.json" /> : <MetricTable caption="표 6. VIF 진단" rows={vifRows} columns={[["variable", "변수"], ["N", "N"], ["VIF", "VIF"]]} source="analysis/descriptive_2020_2025/tables/table_15_vif_diagnostics.csv" condition="후보 텍스트 통제변수" />}</div><p className="caution">상관계수와 VIF는 변수 간 연관성과 중복 가능성을 진단할 뿐 인과효과를 의미하지 않는다. 특히 count와 log count, count와 share, Fog와 평균 문장 길이, past·present·future share는 함께 투입할 때 구조적 중복을 검토해야 한다.</p>{figureError ? <SupplementalError source="/data/figure-data.json" /> : <div className="figure-grid"><FigureShell id="figure-01" number="Figure 1" title="연도별 AI 공시 확산" introduction="AI 관련 직접 문장이 존재하는 firm-year의 연도별 비율을 보여준다." caption="전체 firm-year를 분모로 사용한다. 2023년 전후의 변화는 기술적 관찰이며 인과적 전환점으로 해석하지 않는다." condition="전체 firm-year" source={figureMeta("figure-01").source_file} sourceCsv="/downloads/figure-aggregate-data.csv"><LineFigure id="figure-01-svg" rows={figureData?.aggregate} keys={["ai_disclosure_rate"]} labels={["AI 공시 비율"]} percent ariaLabel="연도별 AI 공시 비율" /></FigureShell><FigureShell id="figure-02" number="Figure 2" title="AI 커뮤니케이션 강도 추이" introduction="전체 표본의 0 포함 평균과 AI 공시기업 조건부 평균을 분리해 표시한다." caption="전체 평균은 AI 미공시 firm-year의 0을 포함하고, 조건부 평균은 ai_disclosure=1만 포함한다. 이는 실제 AI adoption이 아니라 text-based AI communication proxy이다." condition="전체 및 AI 공시 firm-year" source={figureMeta("figure-02").source_file} sourceCsv="/downloads/figure-aggregate-data.csv"><LineFigure id="figure-02-svg" rows={figureData?.aggregate} keys={["mean_ai_sentence_count_all", "mean_ai_sentence_count_disclosers"]} labels={["전체 평균(0 포함)", "공시기업 평균"]} ariaLabel="전체 및 AI 공시기업 AI 직접 문장 수" /></FigureShell><FigureShell id="figure-03" number="Figure 3" title="구체성 추이" introduction="전체 보고서와 AI 직접 문장의 평균 구체성 변화를 비교한다." caption="Brysbaert 기반 측정이며 AI 구체성은 AI 공시 firm-year에 조건부이다. 사전 coverage와 표본 차이를 고려해야 한다." condition="전체 보고서 및 AI 공시 firm-year" source={figureMeta("figure-03").source_file} sourceCsv="/downloads/figure-aggregate-data.csv"><LineFigure id="figure-03-svg" rows={figureData?.aggregate} keys={["whole_report_concreteness", "ai_concreteness"]} labels={["전체 보고서", "AI 직접 문장"]} ariaLabel="전체 보고서와 AI 직접 문장 구체성" /></FigureShell><FigureShell id="figure-04" number="Figure 4" title="시제 구성 변화" introduction="finite tense count를 분모로 과거·현재·미래 표지의 구성을 표시한다." caption="과거는 VBD, 현재는 VBP/VBZ, 미래는 will·shall·'ll/’ll 표지다. 미래지향 언어 전체를 측정하지 않는다." condition="전체 firm-year" source={figureMeta("figure-04").source_file} sourceCsv="/downloads/figure-aggregate-data.csv"><LineFigure id="figure-04-svg" rows={figureData?.aggregate} keys={["past_tense_share", "present_tense_share", "future_tense_share"]} labels={["과거", "현재", "미래 표지"]} percent ariaLabel="연도별 시제 비율" /></FigureShell><FigureShell id="figure-05" number="Figure 5" title="AI 직접 문장의 Loughran–McDonald 언어 추이" introduction="AI 직접 문장에서 positive·negative·uncertainty 범주의 상대 빈도를 비교한다." caption="Loughran–McDonald 금융사전 기반이며 기업의 심리 상태를 직접 측정하지 않는다." condition="AI 공시 firm-year" source={figureMeta("figure-05").source_file} sourceCsv="/downloads/figure-aggregate-data.csv"><LineFigure id="figure-05-svg" rows={figureData?.aggregate} keys={["ai_lm_positive_share", "ai_lm_negative_share", "ai_lm_uncertainty_share"]} labels={["긍정", "부정", "불확실성"]} percent ariaLabel="AI 직접 문장의 Loughran–McDonald 언어 범주" /></FigureShell><FigureShell id="figure-06" number="Figure 6" title="AI 공시·미공시 표준화 평균 차이" introduction="표준화 평균 차이의 방향과 상대적 크기를 0 기준선과 함께 표시한다." caption="연도·산업·기업 규모를 통제하지 않은 단순 집단 비교이며 Welch 검정은 표에서 확인한다." condition="전체 firm-year 단순 비교" source={figureMeta("figure-06").source_file} sourceCsv="/downloads/disclosure-comparison.csv"><EffectSizeFigure id="figure-06-svg" rows={figureData?.comparison} ariaLabel="AI 공시 미공시 표준화 평균 차이" /></FigureShell><FigureShell id="figure-07" number="Figure 7" title="동일 기업 내 전년 대비 변화" introduction="실제 연속연도 pair가 있는 기업의 평균 변화를 변수별로 표시한다." caption="각 변수의 원래 단위를 유지하며 0은 변화 없음이다. 표본 구성 변화와 결측 규칙을 함께 고려해야 한다." condition="연속된 두 연도의 동일 기업" source={figureMeta("figure-07").source_file} sourceCsv="/downloads/figure-within-firm-change-data.csv"><WithinChangeFigure id="figure-07-svg" rows={figureData?.within_change} ariaLabel="동일 기업 내 전년 대비 평균 변화" /></FigureShell></div>}</Section>
