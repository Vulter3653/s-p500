...
import { EffectSizeFigure, FigureShell, LineFigure, WithinChangeFigure } from "./Figures";
...
function Report({ data }) {
  const { summary, definitions, yearly, descriptive, comparison, within, pearson, vif, sampleAudit, sourceManifest, buildMetadata, figureData, figureManifest, docs = {}, supplementalErrors = {} } = data;
  ...
  return <main className="report-app">
    <aside className="toc" aria-label="연구보고서 목차"><div className="toc-brand"><strong>S&amp;P 500</strong><span>연구보고서</span></div><nav><a href="#title">표지</a><a href="#period-settings">기간 설정</a><a href="#abstract">초록</a><a href="#design">1. 연구설계</a><a href="#sample">2. 표본 구축</a><a href="#collection">3. 자료 수집</a><a href="#processing">4. 텍스트 처리</a><a href="#methods">5. 변수 측정</a><a href="#statistics">6. 통계 분석</a><a href="#results">7. 결과</a><a href="#discussion">8. 논의</a><a href="#limitations">9. 한계</a><a href="#reproducibility">10. 재현성</a><a href="#variables">부록 A. 변수 정의</a></nav><div className="toc-meta">VERSION {buildMetadata?.version || "0.13.0"}<br /><small>firm-year · {periodLabel}</small><br /><small>전체 {buildPeriod}</small></div></aside>
    <div className="report-main">
      <section className="title-page" id="title"><p className="kicker">S&amp;P 500 · FORM 10-K · FIRM-YEAR PANEL</p><h1>{periodLabel}년 S&amp;P 500 기업<br /><em>10-K 언어 특성 분석</em></h1><p className="subtitle">AI 공시, 구체성, 시제, Loughran–McDonald 어휘, 수동태와 가독성의 기술통계 및 연관성 분석</p><dl className="title-meta"><div><dt>표시기간</dt><dd>{periodLabel}</dd></div><div><dt>전체 구축기간</dt><dd>{buildPeriod}</dd></div><div><dt>분석단위</dt><dd>firm-year</dd></div><div><dt>선택 관측치</dt><dd>{formatInteger(selectedObservations)}</dd></div><div><dt>Commit</dt><dd><code>{buildMetadata?.git_commit || summary.git_commit}</code></dd></div><div><dt>VERSION</dt><dd>{buildMetadata?.version}</dd></div></dl></section>
      ...
      <Section id="abstract" number="초록" title="연구 요약" kicker="ABSTRACT / EXECUTIVE SUMMARY"><div className="abstract-box"><p>본 화면은 전체 {buildPeriod} 패널 중 {periodLabel}년을 선택하여 S&amp;P 500 기업 Form 10-K의 AI 관련 공시와 문서 언어 특성의 연도별 분포를 기술한다. 선택기간 표본은 {formatInteger(selectedObservations)} firm-year이며 AI 직접 문장이 확인된 관측치는 {formatInteger(selectedAiDisclosures)}건({formatPercent(selectedAiRate)})이다.</p><p>핵심 측정치는 AI 직접 문장 여부와 개수, Brysbaert 구체성, spaCy 기반 시제·수동태, Loughran–McDonald 금융 어휘, Gunning Fog Index 및 보고서 길이다.</p><p className="caution"><strong>해석 범위:</strong> AI 관련 변수는 실제 AI adoption이 아니라 10-K에 나타난 <code>text-based AI communication proxy</code>이다. 평균 차이와 상관관계는 인과효과를 의미하지 않는다.</p></div><SourceNote source="web/public/data/yearly-statistics.json" n={selectedObservations} condition={`${periodLabel} 선택기간`} /></Section>

      <Section id="design" number="1" title="연구설계" kicker="RESEARCH DESIGN"><p>관측 단위는 기업 식별자와 보고연도 조합인 <code>firm-year</code>이다. 전체 구축 패널은 {buildPeriod}이며 화면의 기본 메인 기간은 2020–2025이다.</p><div className="two-column"><div><h3>연구 질문</h3><p>선택한 {periodLabel}년 동안 AI 관련 직접 공시가 얼마나 확산되었고, 언어 특성이 어떻게 변화했는지 기술한다.</p></div><div><h3>AI communication proxy</h3><p><code>ai_disclosure</code>는 기존 AI matcher가 추출한 직접 문장이 하나 이상일 때 1이며 실제 기술 도입을 직접 측정하지 않는다.</p></div></div></Section>
      ...
      <footer className="report-footer">기본 표시기간 {defaultStart}–{defaultEnd} · 전체 구축기간 {buildPeriod} · VERSION {buildMetadata?.version || "0.13.0"} · commit <code>{buildMetadata?.git_commit || summary.git_commit}</code></footer>
    </div>
  </main>;
}
