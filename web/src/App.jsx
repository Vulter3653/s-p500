  ...
  return (
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <section className="paper-tables" aria-label="논문용 표 형식"><article className="panel paper-table-panel"><div className="panel-heading"><div><p className="eyebrow">TABLE 1 · DESCRIPTIVE STATISTICS</p><h2>기술통계</h2></div><span className="status">전체 firm-year</span></div><div className="table-wrap"><table className="paper-table"><caption className="sr-only">Table 1 기술통계</caption><thead><tr><th>변수</th><th>Mean</th><th>Std Dev.</th><th>Q1</th><th>Median</th><th>Q3</th><th>N</th></tr></thead><tbody>{analysisSummary.descriptiveTable.map((item) => <tr key={item.variable}><th scope="row"><code>{item.variable}</code><small>{item.label}</small></th><td>{tableStat(item.mean, item.kind)}</td><td>{tableStat(item.sd, item.kind)}</td><td>{tableStat(item.q1, item.kind)}</td><td>{tableStat(item.median, item.kind)}</td><td>{tableStat(item.q3, item.kind)}</td><td>{item.n.toLocaleString("ko-KR")}</td></tr>)}</tbody></table></div><p className="table-note">주: share 변수는 0–1 원자료를 백분율로 표시했습니다. AI 문장 구체성은 유효한 AI 공시 firm-year만 포함하므로 N이 다릅니다.</p></article><article className="panel paper-table-panel"><div className="panel-heading"><div><p className="eyebrow">TABLE 2 · DETERMINANTS DESIGN</p><h2>텍스트 특성 결정요인 분석 설계</h2></div><span className="status">계수 미추정</span></div><div className="table-wrap"><table className="paper-table model-table"><thead><tr><th>종속변수</th><th>모형</th><th>핵심 설명변수</th><th>고려할 통제·고정효과</th><th>상태</th></tr></thead><tbody><tr><th scope="row"><code>report_word_count</code><small>보고서 길이</small></th><td>패널 회귀</td><td><code>report_year</code>, <code>ai_disclosure</code></td><td>기업·연도 고정효과, 재무 통제변수</td><td>후속 분석</td></tr><tr><th scope="row"><code>fog_index</code><small>가독성</small></th><td>패널 회귀</td><td><code>report_year</code>, <code>ai_sentence_count</code></td><td>기업·연도 고정효과, 보고서 길이</td><td>후속 분석</td></tr><tr><th scope="row"><code>whole_report_concreteness</code><small>구체성</small></th><td>패널 회귀</td><td><code>report_year</code>, <code>ai_disclosure</code></td><td>기업·연도 고정효과, 문서 통제변수</td><td>후속 분석</td></tr><tr><th scope="row"><code>ai_sentence_count</code><small>AI 공시 강도</small></th><td>count/log 모형</td><td><code>report_year</code>, 산업·기업 특성</td><td>zero 처리, 과산포 검토</td><td>후속 분석</td></tr></tbody></table></div><p className="table-note">주: 현재 단계는 기술통계·상관관계까지만 수행했습니다. 계수, t-stat, 표준오차 및 유의확률을 임의로 채우지 않았으며 재무 통제변수도 새로 수집하지 않았습니다.</p></article></section>
