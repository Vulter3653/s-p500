...
test("audit all professor figures against dot-to-dot v3", async ({ page }) => {
  ...
  const diagnostic = await page.evaluate(() => {
    ...
    allPoints.forEach((point) => {
      ...
      if (!Number.isFinite(raw)) {
        pointErrors.push(
          `non-finite raw point: ${point.dataset.series || "?"}/${point.dataset.year || "?"}`,
        );
      }
      if (!finiteAttr(point, "cx") || !finiteAttr(point, "cy")) {
        pointErrors.push(
          `non-finite point coordinate: ${point.dataset.series || "?"}/${point.dataset.year || "?"}`,
        );
      }
      if (!normalizeColor(point.getAttribute("fill"))) {
        pointErrors.push(
          `missing point color: ${point.dataset.series || "?"}/${point.dataset.year || "?"}`,
        );
      }
    });
    ...
    allLabels.forEach((label) => {
      ...
      if (!Number.isFinite(raw)) {
        labelErrors.push(
          `non-finite label raw value: ${label.dataset.series || "?"}/${label.dataset.year || "?"}`,
        );
        return;
      }
      if (
      ...
      const match = (label.textContent || "")
        .replace(/,/g, "")
        .match(/-?\d+(?:\.\d+)?/);
      if (!match) {
        labelErrors.push(
          `non-numeric visible label: ${label.dataset.series || "?"}/${label.dataset.year || "?"}`,
        );
        return;
      }
    ...
    figures.forEach((figure, figureIndexZero) => {
      const displayNumber =
        figure.querySelector(".figure-number")?.textContent?.trim() ||
        `DOM Figure ${figureIndexZero + 1}`;
      const figureSvgs = Array.from(figure.querySelectorAll("svg.figure-svg"));
      if (figureSvgs.length === 0) {
        missingSvgErrors.push(displayNumber);
        return;
      }

      if (figure.scrollWidth > figure.clientWidth + 1) {
        figureOverflowErrors.push(displayNumber);
      }
      Array.from(figure.querySelectorAll(".figure-plot")).forEach((plot, plotIndex) => {
        if (plot.scrollWidth > plot.clientWidth + 1) {
          plotOverflowErrors.push(`${displayNumber}/plot-${plotIndex + 1}`);
        }
      });
      ...
      figureSvgs.forEach((svg, svgIndex) => {
        auditedSvgCount += 1;
        const panelName = `${displayNumber}/svg-${svgIndex + 1}`;

        const axisTitles = svg.querySelectorAll(".chart-axis-title");
        const axisTicks = svg.querySelectorAll(".chart-x-tick, .chart-y-tick");
        if (axisTitles.length < 1) axisErrors.push(`${panelName}: missing axis title`);
        if (axisTicks.length < 2) axisErrors.push(`${panelName}: insufficient axis ticks`);

        // Figure 6 is the horizontal SMD figure, not a year-to-year line-series chart.
        if (displayNumber === "Figure 6") return;
        ...
        if (svg.querySelectorAll("polyline").length !== 0) {
          legacyPolylineErrors.push(panelName);
        }

        const pointColors = new Set(
          yearPoints.map((point) => normalizeColor(point.getAttribute("fill"))),
        );
        pointColors.forEach((color) => {
          if (color && !legendColors.has(color)) {
            legendColorErrors.push(`${panelName}: point color ${color} missing from legend`);
          }
        });

        const labels = Array.from(
          svg.querySelectorAll(
            "text.chart-value-label[data-series][data-year][data-label-value]",
          ),
        );
        const labelBySeriesYear = new Map(
          labels.map((label) => [
            `${label.dataset.series || ""}@@${label.dataset.year || ""}`,
            label,
          ]),
        );
        ...
        yearPoints.forEach((point) => {
          ...
          const key = `${series}@@${point.dataset.year || ""}`;
          const label = labelBySeriesYear.get(key);
          if (!label) {
            pointLabelPairErrors.push(`${panelName}: missing label ${key}`);
          } else if (
            Math.abs(Number(point.dataset.pointValue) - Number(label.dataset.labelValue)) > 1e-12
          ) {
            pointLabelPairErrors.push(`${panelName}: raw mismatch ${key}`);
          }
        });
        ...
        bySeries.forEach((points, series) => {
          ...
          points.forEach((point) => {
            ...
            if (seenYears.has(year)) {
              duplicatePointErrors.push(`${panelName}: ${series}/${year}`);
            }
            seenYears.add(year);
          });
          ...
          for (let i = 1; i < points.length; i += 1) {
            ...
            if (endYear === startYear + 1) {
              expected.set(`${series}@@${startYear}@@${endYear}`, {
                prev,
                curr,
                series,
              });
        ...
        const connectors = Array.from(
          svg.querySelectorAll('line[data-dot-connector="true"]'),
        );
        ...
        connectors.forEach((line) => {
          ...
          if (seenConnectorKeys.has(key)) {
            connectorKeyErrors.push(`${panelName}: duplicate connector ${key}`);
          }
          seenConnectorKeys.add(key);

          if (endYear !== startYear + 1) {
            connectorKeyErrors.push(`${panelName}: non-adjacent connector ${key}`);
          }
          if (!target) {
            connectorKeyErrors.push(`${panelName}: unexpected/gap connector ${key}`);
            return;
          }
          ...
          endpointPairs.forEach(([attr, expectedValue]) => {
            if (Math.abs(Number(line.getAttribute(attr)) - Number(expectedValue)) > 1e-9) {
              connectorEndpointErrors.push(
                `${panelName}: ${key} ${attr}=${line.getAttribute(attr)} expected=${expectedValue}`,
              );
            }
          });
          ...
          if (stroke !== startFill || stroke !== endFill) {
            connectorColorErrors.push(
              `${panelName}: ${key} stroke=${stroke} start=${startFill} end=${endFill}`,
            );
          }

          const dash = (line.getAttribute("stroke-dasharray") || "").trim();
          if (series === "전체" && dash !== "8 5") {
            connectorDashErrors.push(`${panelName}: 전체 connector is not dashed ${key}`);
          }
          if (series !== "전체" && dash !== "") {
            connectorDashErrors.push(`${panelName}: non-전체 connector is dashed ${key}`);
          }
        });

        expected.forEach((_value, key) => {
          if (!seenConnectorKeys.has(key)) {
            connectorKeyErrors.push(`${panelName}: missing connector ${key}`);
          }
        });
      });
    });

    const figure3 = figures.find(
      (figure) => figure.querySelector(".figure-number")?.textContent?.trim() === "Figure 3",
    );
    ...
    const figure3GapBridges = figure3
      ? Array.from(figure3.querySelectorAll('line[data-dot-connector="true"]')).filter(
          (line) => {
    ...
    return {
      scriptPresent: Boolean(
        document.querySelector("script#production-dot-to-dot-line-sync-v3"),
      ),
      legacyScriptPresent: Boolean(
        document.querySelector("script#production-gap-safe-line-sync-v2"),
      ),
      ...
      lineSeriesSvgCount,
      observedGapCount,
      figure3Present: Boolean(figure3),
  ...
  expect(diagnostic.svgCount).toBeGreaterThanOrEqual(diagnostic.figureCount);
  expect(diagnostic.auditedSvgCount).toBe(diagnostic.svgCount);
  expect(diagnostic.missingSvgErrors).toEqual([]);
  ...
  expect(diagnostic.figure3Present).toBe(true);
  ...
  // Console errors are preserved in the diagnostic artifact but are not an
  // integrity assertion because browser/extension/network noise is outside the SVG contract.
});
